...
public class Joystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, IMovementSource
{
    [HideInInspector] public RectTransform JoystickRect;
    [RequiredField] public RectTransform KnobRect;
    [field: SerializeField] public UnityEvent<Vector2> OnMove { get; set; } = new();
    ...
    void UpdateJoystickPosition(PointerEventData eventData)
    {
        ...
        Vector2 normalizePosition = clampedPosition / maxKnobDistance;
        OnMove.Invoke(normalizePosition.magnitude > Deadzone ? normalizePosition : Vector2.zero);
    }
